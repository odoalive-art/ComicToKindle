...
import { ArrowLeft, ArrowRight, ArrowUp, ArrowUpRight, BadgeCheck, BookOpen, Calendar as CalendarIcon, Calculator, CheckCircle2, ChevronDown, CircleFadingArrowUp, Clock3, Component, Copy, ExternalLink, FileText, Info, FolderOpen, GitBranch, Grid2X2, HardDrive, Home, Inbox, Loader2, MoreHorizontal, Package, Palette, Plus, Rows3, Ruler, Search, Settings, Terminal, AlertCircle, ChevronRight, Slash, Type } from 'lucide-react'
...
import { Menubar, MenubarContent, MenubarItem, MenubarMenu, MenubarTrigger, MenubarCheckboxItem, MenubarRadioGroup, MenubarRadioItem, MenubarSeparator, MenubarShortcut, MenubarSub, MenubarSubTrigger, MenubarSubContent } from '@/components/ui/menubar'
...
import { Sidebar, SidebarContent, SidebarFooter, SidebarGroup, SidebarGroupAction, SidebarGroupLabel, SidebarHeader, SidebarInset, SidebarMenu, SidebarMenuBadge, SidebarMenuButton, SidebarMenuItem, SidebarProvider, SidebarRail, SidebarSeparator, SidebarTrigger, useSidebar } from '@/components/ui/sidebar'
...
import { ContextMenu, ContextMenuCheckboxItem, ContextMenuContent, ContextMenuGroup, ContextMenuItem, ContextMenuRadioGroup, ContextMenuRadioItem, ContextMenuSeparator, ContextMenuShortcut, ContextMenuSub, ContextMenuSubContent, ContextMenuSubTrigger, ContextMenuTrigger } from '@/components/ui/context-menu'
...
import { installedShadcnComponentSlugs, type LocalizedText, mirroredShadcnDocs, shadcnComponents, shadcnDocsSource, type ShadcnComponent, type ShadcnDocBlock } from '@/data/shadcn-docs'

import { uiText, type LanguageMode } from '../i18n'

// ---------- 演示页辅助函数（从 App 迁入，仅演示用）----------
...
// ---------- 开发期演示页入口（组件 / 基础规范 / 设计组件）----------
// 由 App.tsx 经 React.lazy 仅在 dev 构建载入；自带顶栏，封装选中态与过滤。
type ShowcaseViewId = 'design-components' | 'foundation-standards' | 'app-components'
...
export default function DevShowcase({
  ...
  const appComponents = shadcnComponents.filter((item) =>
    installedShadcnComponentSlugs.has(item.slug)
  )
  const components = activeView === 'app-components' ? appComponents : shadcnComponents
  const selectedSlug =
    activeView === 'app-components'
      ? appComponents.some((c) => c.slug === selectedComponentSlug)
        ? selectedComponentSlug
        : appComponents[0]?.slug || 'button'
      : selectedComponentSlug
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center gap-1.5">
          <span className="text-sm font-semibold text-foreground">{text.nav[activeView]}</span>
        </div>
        ...
        {isComponentView ? (
          ...
          >
            <a
              ...
              href={
                mirroredShadcnDocs[selectedSlug]?.officialUrl ??
                `https://ui.shadcn.com/docs/components/radix/${selectedSlug}`
              }
              rel="noreferrer"
              target="_blank"
            >
            ...
            <ShadcnSourceDialog
              locale={locale}
              selectedPath={mirroredShadcnDocs[selectedSlug]?.sourcePath}
            />
          </div>
        ) : null}
      </header>
      ...
      {isComponentView || activeView === 'app-components' ? (
        ...
        <ScrollArea className="min-h-0 flex-1">
          <FoundationStandardsView locale={locale} />
        </ScrollArea>
      ) : (

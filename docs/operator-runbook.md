...
**这不是 bug，是预期行为**。「已增强」角标走 `upscale:peek` IPC，只要 `enhance()` 成功返回非原图路径就点亮，说明 waifu2x **对两部都真的跑过且成功输出**（可在 `userData/upscaled/` 看到两部各自的 2× PNG）。能否「看出来」取决于**源图分辨率相对屏幕显示尺寸**：

- 阅读器双页并排，每页屏上约 ~850px 宽。
- **源图低于显示尺寸**（如 850×1200）：1:1 顶着显示、本就偏软 → 2× + 降噪后相当于超采样，肉眼明显变锐。
- **源图已高于显示尺寸**（如 1360×1920）：本来就要被浏览器缩小显示、细节已足 → 2× 再缩回显示等于没变，降噪对清晰扫描图增益也极小。

判定方法：`sips -g pixelWidth -g pixelHeight <源图>` 看源图宽度；远小于显示宽度才会有可见收益。这是源素材决定的，调参（含 `denoise:3` 最高档）也无法在已高清的源上「补」出更多细节。

> 一句话排查结论：先确认源图宽度。高清源（≥1400px 宽）增强照常运行但屏上无可见差别，属正常；只有低清源才看得出。

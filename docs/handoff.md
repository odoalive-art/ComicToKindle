...
- **排序 UI**：右键菜单新增“上移 / 下移”，顶层部调用 `reorderSeries`，顶层散卷和部内卷册调用 `reorderBooks`，排序只写 manifest，不移动桶目录。图标视图、列表视图、部内网格都已接入。
- 验证：`npm run typecheck` 通过；`npm run build` 待本轮结束前执行。
- 待办：移除旧文件视图和本地文件操作 IPC。

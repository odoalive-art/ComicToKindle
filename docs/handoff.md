...
ComicToKindle **核心闭环已打通**：漫画库浏览 → 阅读器 → 卷册转 Kindle 固定版式 EPUB → 归档 → SMTP 投递到 Kindle 邮箱。最近一轮（2026-06-19）集中打磨库视图交互：多选/框选批量转换、顶栏图标化、侧栏开合按钮迁移、窄屏适配。
...
浏览/阅读/转换（图片→EPUB）/归档/SMTP 投递闭环已实现；仍未实现：元数据存储/索引、压缩包（CBZ/CBR/PDF）来源、图像放大、转换队列持久化、转换后自动投递。

...
ComicToKindle **核心闭环已打通**：漫画库浏览 → 阅读器 → 卷册转 Kindle 固定版式 EPUB → 归档 → 投递到 Kindle（SMTP 邮件 / Send to Kindle 网页通道二选一）。最近一轮（2026-06-21）补齐**压缩包来源**（加分卷/解压进度/防挂起）、**封面缩略图缓存**、**书籍元数据**（书名「漫画名+卷册」+ 作者 + 转换确认弹窗）与**每部信息编辑**。
...
浏览/阅读/转换（图片 + CBZ/ZIP/CBR/RAR/7z 压缩包含分卷→EPUB，书名+作者元数据）/归档/投递（SMTP + Send to Kindle 网页通道）闭环已实现；仍未实现：元数据存储/索引、PDF/EPUB 来源、图像放大、转换队列持久化、转换后自动投递。
...
8. 批量转换的逐卷书籍信息确认（当前批量不弹窗，直接用合成书名）。

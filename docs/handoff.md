...
### 2026-06-28 阶段（AI 放大集成 + 状态反馈修复，合入并推送 `main`）

- 由 Claude（集成者）把 A/B/C 三线合入 `main`（`cbfd680`），三线均在同一工作树顺序产出、无跨界冲突；`typecheck`/`build`/`pack:doctor` 全过。
- 阅读增强状态反馈修复（`3c5ca44`）：把「fetch 读 `comic://` 响应头判状态」改为 `upscale:peek` IPC（复用增强缓存 + inFlight）。**根因**：Chromium 禁止 dev 源跨源 `fetch(comic://…)`，CORS 在拿到 Response 前拒绝，响应头读不到 → 角标恒「失败」；图片仍走稳定的 `<img …&enhance=1>`。排查与修复由 Codex 完成。
- 已推送至 `origin/main`。roadmap 把「图像增强」拆为「已上线·阅读时 AI 放大」+「探索中·转换时图像增强」；本工单 `docs/plan-extensions-upscale.md` 标记已实现。
...
产品主闭环为：托管漫画库浏览 → 阅读器 → 卷册转 Kindle 固定版式 EPUB → 归档 → 投递到 Kindle（SMTP 邮件 / Send to Kindle 网页通道二选一）。漫画库为 Eagle 式 `.ctklib` 托管库包；库模型为「书（单册，自带书名+作者）+ 文件夹（纯收纳，无作者）」。
...
- 修复：`mac.identity` 改为 `"-"` 仅用于进入 `scripts/sign-mac.cjs`；钩子按 `CTK_SIGN_IDENTITY` 直接执行自签 `codesign --deep --timestamp=none`，不传变量时执行 ad-hoc `--sign -`。`pack:doctor` 已增加该配置的防回归检查。
- 产物验收：最终 zip 解压后为 `Authority=ComicToKindle Self-Signed` / `Identifier=com.comictokindle.app`，`codesign --verify --strict` 通过；`latest-mac.yml` 的 size/sha512 与最终 zip 一致。
- 升级实测：将 `0.1.0-beta.3` 安装到 `/Applications`，发布 `0.1.0-beta.4` 后重开 A；客户端从公开 GitHub Release 发现并下载 B，点「立即重启更新」后 Squirrel.Mac 成功替换 app 并重启到 beta.4，更新后签名验证仍通过。
- 发布：仓库已切为 public；[v0.1.0-beta.3](https://github.com/odoalive-art/ComicToKindle/releases/tag/v0.1.0-beta.3) 和 [v0.1.0-beta.4](https://github.com/odoalive-art/ComicToKindle/releases/tag/v0.1.0-beta.4) 已正式发布。

### 2026-06-27 阶段（1.0 多线集成：合并 A 发布管线 + B 转换稳定性 + C 引导向导）

由 Claude（集成者）在 `feat/convert-workbench` 上合并三条并行线（B 条目见下节）：

- **A 线（发布管线，`feat/release-pipeline`）**：新增 `src/main/updater.ts`（electron-updater 检查/下载/重启提示，仅打包后生效，主进程原生对话框），`index.ts` 接线；`electron-builder.yml` mac 加 zip target + publish 改 GitHub feed；`build-mac-dmg.mjs` 支持 `CTK_SIGN_IDENTITY` 自签 + `CTK_PUBLISH` 上传；`release-mac.mjs` 改为「自签 + 上传 GitHub Release」流程带前置校验；`pack-doctor.mjs` 校验项同步。发布策略：自签证书（免费）+ 自动更新，首次一次性 Gatekeeper 警告；Developer ID 公证为可选后续。
- **C 线（引导 + 投递向导，`feat/onboarding-delivery`）**：新建 `src/renderer/src/onboarding/`（欢迎/建库/导入第一本/投递四步 + `App.tsx` first-run gate）、`delivery-wizard/`（SMTP 模板 + 应用授权码说明 + Amazon 认可发件人引导 + `testSMTP` 自检）。Claude 修复验证发现的 2 个 bug：欢迎页按钮重叠（flex 加 `min-h-0`）、投递步骤死胡同（加「稍后配置」出口）。
- **冲突解决**：`i18n.ts` 同时保留 B 的 `laneBConvertText` + C 的 `onboardingText` 两块；`handoff.md` 保留全历史（C 曾误整体覆盖，已恢复）；`package-lock.json` 重新生成。
- 验证：合并后 `npm run typecheck && build` 通过。A 线自动更新真实升级已于 2026-06-27 通过；C 引导实走与 B 转换边界总质检仍需单独跟进。

### 2026-06-27 阶段（B 线：转换稳定性 + 自动投递 main 层，分支 `feat/convert-stability`）

当前状态：

- B 线允许范围内的转换稳定性、来源错误路径、自动投递 main 层与网页注入双语已完成；未新增依赖。

已完成：

- **大 PDF**（`src/main/document.ts`）：普通 PDF 继续交给 utility process 渲染；渲染前逐页预检并回传单调递增进度。超大页按 `24,000,000` 像素 / `12,000px` 边长上限自适应降采样，逐页让出 event loop；utility process 遇特殊字体或 Canvas 环境错误时回退 main 逐页渲染。密码保护、空 PDF、损坏结构和渲染失败均返回带恢复建议的稳定错误前缀。
- **异常图片 / 空卷**（`src/main/convert.ts`、`src/main/artifacts.ts`）：损坏图片不再静默跳过，直接指出文件名并提示替换或删除后重试；空卷、缺书名有明确中英错误。并发图片任务会等待工作线程收口后再清临时目录；转换失败清理本轮已生成输出，打包输出流错误会正常 reject。
- **压缩包失败路径**（`src/main/archive.ts`）：7-Zip stderr 纳入诊断；区分错误密码、缺分卷、损坏/不支持压缩方法、不可读文件。只有明确加密错误才进入密码框，损坏包不再误导用户反复输密码；无图包仍走既有 `NO_IMAGES` 提示。
- **重排 EPUB**（`src/main/document.ts`）：无可用图片时返回 `EPUB_REFLOW_UNSUPPORTED`，明确说明纯文本/重排 EPUB 暂不支持，并给出转为图片型 EPUB / 图片目录的恢复路径；损坏 EPUB 单独提示重新导出。
- **转换后自动投递 main 层**（`src/main/artifacts.ts`、`src/main/deliver.ts`、`src/main/webpush.ts`）：`settings.json > delivery` 支持 `autoDeliveryEnabled` 与 `autoDeliveryChannel ('smtp' | 'webpush')`。转换产物落清单后按当前设置触发：SMTP 直接发送；网页通道自动打开并填入文件，最终 Send 仍由用户确认。失败不回滚转换，产物置 `failed`，归档原有手动发送/网页推送入口可重试；同一产物 SMTP 并发请求会合并。
- **网页注入双语**（`src/main/webpush.ts`、`src/renderer/src/i18n.ts`）：注入横幅、蒙层、关闭确认框全部读取 `// === lane-B convert ===` 的中英文案；手动打开时从 renderer 的 `comic-to-kindle-language` 同步语言，自动投递沿用转换发起窗口语言。
...
- `src/main/convert.ts`、`src/main/document.ts`、`src/main/archive.ts`
- `src/main/artifacts.ts`、`src/main/deliver.ts`、`src/main/webpush.ts`
- `src/renderer/src/i18n.ts`（仅 lane-B 锚点追加）
- `docs/handoff.md`

验证：

- `npm run typecheck` 通过。
- `npm run build` 通过；main / preload / renderer 均成功产出，无动态导入警告。

注意事项：

- 自动投递设置默认关闭；旧 `settings.json` 无需迁移。
- 网页自动投递只负责打开并填入文件，不自动点击 Amazon Send，保留人工确认。
- 本轮未改 renderer 组件、preload 实现、打包/发布配置、`package.json` 或 lockfile。
...
该仓库已迁出 iCloud Drive，且是唯一工作副本。旧的 iCloud 副本已弃用，不要在其中构建或修改代码。

...
**Git 推送（2026-06-21 已解决）**：macOS 自带 `/usr/bin/git` 用 LibreSSL，HTTP/2 上传到 GitHub 会被重置（`LibreSSL SSL_ERROR_SYSCALL`，读操作/curl 正常、push 必挂）。已设全局 `git config --global http.version HTTP/1.1` 修复，正常 `git push` 即可（不必再手动 `!` 或加参数）。提交身份也已规整为全局 `user.name=olart` / `user.email=247497017+odoalive-art@users.noreply.github.com`（GitHub 隐私 noreply，关联 `odoalive-art`、不漏真实邮箱/主机名）。
...
浏览/阅读/转换（图片 + CBZ/ZIP/CBR/RAR/7z 压缩包含分卷→EPUB，书名+作者元数据）/队列持久化（含中断恢复）/应用内文件整理第一片（右键重命名/移动/删除到废纸篓/新建文件夹）/归档/投递（SMTP + Send to Kindle 网页通道）闭环已实现；仍未实现：元数据存储/索引、PDF/EPUB 来源、图像放大、转换后自动投递、文件整理第二片（拖拽/批量重命名等，见记忆 `library-file-manager-direction`）。
...
5. 视情况引入漫画元数据库/索引，替代每次实时扫描。
6. 阅读器增强：双页「封面单独成页」、缩放/适配宽度、沉浸模式、Home/End 跳首末页。
7. 网页推送小打磨（可选）：蒙层/确认框文案接 i18n（当前 main 进程硬编码中文）；关闭确认改成「仅有已填入未发送文件时才弹」。
8. 批量转换弹窗已实现（共享漫画名/作者 + 书名预览）；若需**逐卷**书籍信息编辑可再做。

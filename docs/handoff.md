...
- **部内刷新**：导入完成后复用 `refreshAfterFileop()`，刷新顶层 manifest、当前部卷册列表和已展开部缓存，避免部内导入后页面不更新。
- **实体列表防溢出**：`EntityListDialog` 增加 `min-w-0`、文本截断/折行、右侧操作区收缩保护，修复库内回收站长标题 + Restore 按钮横向撑出弹窗的问题；协作规则同步要求左右布局优先压缩信息文本，不挤走交互入口。
- **文案与文档**：补中英文 UI 文案；更新 `AGENTS.md`、`docs/roadmap.md`、`docs/release-notes.md`、`docs/plan-import-enhancements.md`。
- 验证：`npm run typecheck`、`npm run build` 通过。待完全重启 `npm run dev` 后真机验证 A 三种目标、B 单/多路径拖放，以及打包 dmg 中 `webUtils.getPathForFile`。

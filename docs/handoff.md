...
浏览/阅读/转换（图片 + CBZ/ZIP/CBR/RAR/7z 压缩包含分卷→EPUB，书名+作者元数据）/队列持久化（含中断恢复）/归档/投递（SMTP + Send to Kindle 网页通道）闭环已实现；仍未实现：元数据存储/索引、PDF/EPUB 来源、图像放大、转换后自动投递、应用内本地文件夹整理（漫画库文件管理器化的远期目标，见记忆 `library-file-manager-direction`）。

Codex、Claude Code、Antigravity 接力开发前应先读 `docs/agent-collaboration.md`。

## 下一步建议

1. **应用内本地文件夹整理**（漫画库文件管理器化的下一大步）：移动/重命名/重组卷册，需 main 进程文件操作 IPC + 越权校验。详见记忆 `library-file-manager-direction`。
2. 转换后自动投递（可选开关）。

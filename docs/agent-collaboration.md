...
- 改内测分发内容、用户可见能力摘要或版本说明：更新 `docs/release-notes.md`。
- 改产品方向、计划项或能力状态：更新 `docs/roadmap.md`。
...
- 左右布局组件必须防横向溢出：容器和文本区使用 `min-w-0`，长标题、路径、说明使用 `truncate`、`break-words` 或合理折行；右侧按钮、菜单、图标按钮等交互入口保持可见，不被长文本挤出父级，也不要给操作区设置会裁剪按钮文案的固定窄宽或 `max-width`。

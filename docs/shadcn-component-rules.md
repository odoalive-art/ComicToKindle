...
*   **完整登记**：上述提取到的每一个独立变体预览，都必须在 [shadcn-docs.ts](../src/renderer/src/data/shadcn-docs.ts) 的 `mirroredShadcnDocs` 中的 `sections` 里以 `{ type: 'preview', name: '<example-slug>' }` 结构进行逐一注册，并在 [App.tsx](../src/renderer/src/App.tsx) 中实现独立的分支渲染。
...
每个组件在梳理时，必须分清以下三种状态，并在 [shadcn-docs.ts](../src/renderer/src/data/shadcn-docs.ts) 中准确标记：
...
在 [App.tsx](../src/renderer/src/App.tsx) 中编写 Live 预览时，需根据组件的安装状态，按以下三个级别进行高保真呈现：
...
1.  **官方案例完整性比对 (Examples Check)**：
    ...
    *   **比对列表**：逐项检查官方 Examples 下的每个 heading/ComponentPreview，与本地 [shadcn-docs.ts](../src/renderer/src/data/shadcn-docs.ts) 中注册的 `sections` 列表是否在数量和 Slug 命名上 100% 一致。
    *   **多预览分支检查**：检查 [App.tsx](../src/renderer/src/App.tsx) 中是否有针对上述每一个案例 Slug 的条件渲染（分支代码），严禁出现配置了 Examples 但预览回退到 default 兜底或报错的情况。
2.  **双语校验**：

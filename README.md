...
- 真实本地漫画库浏览：通过 main 进程递归扫描目录（任意深度自动判定部/卷、隐藏空目录）、`comic://` 协议加载封面（封面走 480px webp 缩略图缓存）；书架视图多级下钻 + 面包屑；库根目录持久化；每部名称/作者可在应用内编辑（右键网格 / 顶栏铅笔，存 `settings.json` 的 `seriesMeta`，不改本地文件夹名）。
...
- 库视图交互＝文件管理器式（对标 Eagle），内容区统一使用网格交互：书架根支持智能部/卷下钻；左侧边栏文件夹树相当于目录面包屑/定位器，点文件夹后同一套网格忠实列出磁盘直接内容（子文件夹、可读漫画单文件、普通文件、图片文件）。

...
- 真实本地漫画库浏览：通过 main 进程扫描目录、`comic://` 协议加载封面（封面走 480px webp 缩略图缓存），按「部 / 卷册」两级展示；库根目录持久化；每部名称/作者可在应用内编辑（右键网格 / 顶栏铅笔，存 `settings.json` 的 `seriesMeta`，不改本地文件夹名）。
...
- 库视图交互＝文件管理器式（对标 Eagle）：**双击**进入（部→卷、卷→阅读器），**单击**选中（整卡为单位）；卷多选支持 Cmd/Ctrl 累加、空白框选、Cmd·Ctrl+A，空白单击取消、ESC 退出；选中后顶栏「转换所选」批量转换（单本/多本分别弹确认弹窗）。
- 应用内文件整理（文件管理器化第一片）：库网格右键即可**重命名 / 移动到另一部 / 删除到废纸篓**（卷与部），顶栏「新建文件夹」；均为真·本地文件操作，统一越权校验，删除走系统废纸篓可还原。
...
- 尚未实现：漫画元数据存储/索引、PDF/EPUB 来源、图像 AI 放大、转换后自动投递；文件整理第二片（拖拽移动、批量重命名、移动到任意层级、移动框内「新建并移入」）。

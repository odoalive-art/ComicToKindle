...
- 尚未实现：漫画元数据存储/索引、图像 AI 放大、转换后自动投递；文件整理第二片（拖拽移动、批量重命名、移动到任意层级）。

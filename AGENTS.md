...
ComicToKindle 是一个桌面应用项目，用于本地漫画库管理、Kindle 转换流程和投递工具。核心闭环已打通：应用壳、卷册阅读器（快速翻页进度条、沉浸全屏、图片目录/压缩包/图片型 EPUB 的本地 waifu2x 阅读增强）、卷册（图片目录 + CBZ/ZIP/CBR/RAR/7z 压缩包，含加密 zip 与多卷分卷；PDF 单文件；图片型 EPUB）转 Kindle 固定版式 EPUB（sharp + archiver，书名为单一字段、每本书自带书名+作者，转换工作台为独立模态弹窗）、转换队列持久化（`userData/queue.json`，关窗即退；重启后未完成任务标为 interrupted 等用户确认继续 + 孤儿 tmp 清扫）、产物归档、投递到 Kindle（SMTP 邮件 nodemailer + safeStorage 加密凭据 / Send to Kindle 网页通道 ≤200MB 二选一）。漫画库已改为 Eagle 式 App 独占 `.ctklib` 托管库包（`library.json` + `books/<bookId>/book.json` + `source.*`/`images/`）：支持新建/打开库包、扫描导入源、复制卷册入库、导入预览、导入时选择散卷/已有文件夹/新建文件夹、外部拖放文件或文件夹导入、散卷展示、文件夹 CRUD、多级分组下钻、书归属、每本书书名+作者编辑（含多选批量设作者）、文件夹重命名、右键上移/下移排序、导入后删除源选项、书软删除到库内 `trash/`、库内回收站列表/还原/清空。模型已简化为「书（单册，自带书名+作者）+ 文件夹（纯收纳，无作者，作者跟书走）」；阅读/转换复用现有桶路径取图。尚未实现：元数据索引、纯文本/重排 EPUB、PDF 阅读增强、转换时图像增强。转换后自动投递暂缓，当前投递重心是 Send to Kindle 网页通道。数据层与压缩包/文档来源/转换/队列持久化/投递/网页推送/图像增强层详见 `docs/architecture.md`。
...
当前仓库已迁出 iCloud Drive，且是唯一工作副本。旧的 iCloud 副本已弃用，不要在其中构建或修改代码。
...
`漫画库` 是真实功能，采用桌面书架式交互（对标 Eagle）：内容区通过 `library:scan/listVolumes` 的 manifest 投影渲染部（folder）与散卷（book），支持多级分组下钻、面包屑、双击进入/打开、单击选中整卡、右键整理、卷册框选/多选和空白取消；顶层「所有漫画」框选只纳入书卡，文件夹仍保持单选，避免批量转换/移动误作用到文件夹。可读卷册除图片文件夹外，也支持 CBZ/ZIP/CBR/RAR/7z 压缩包（含加密 zip 与分卷，共享密码池，解压进度）、PDF 单文件和图片型 EPUB；压缩包和 EPUB 阅读前经 `window.api.archive.prepare/unlock` 准备成图片缓存，PDF 阅读走 Chromium 内置查看器、转换时才渲染页面缓存。统一图片阅读器以顶栏页码作为快速翻页入口，点击后展开临时 Slider（拖动只预览页码、松手跳转，双页模式对齐跨页），不在漫画画面上常驻控件；支持 `F`/按钮进入沉浸全屏、`Esc` 依次关闭翻页面板和退出全屏，全屏控件随鼠标移动出现并在静置后隐藏。AI 开启时原图与增强图使用双层渲染，原图邻页始终预热，增强按后两跨页再前一跨页串行预取；拖动进度条暂停 AI 预热，增强完成后 200ms 淡入，避免换页黑闪。顶栏 AI 原图对比与全屏按钮使用 Lucide `Blend`、`Fullscreen`/`Shrink`。导入入口包括顶栏按钮和库内容区外部拖放，都会进入同一个扫描/预览弹窗；预览中可选择落到散卷、已有顶层文件夹或新建文件夹。文件夹名、每本书的书名/作者、分组归属和排序均写入 `.ctklib` 的 manifest / book manifest，不改用户原始文件；右键整理走 `library:setBookMeta/setBooksAuthor/renameSeries/deleteSeries/assignBooks/reorderSeries/reorderBooks/trashBooks` 等托管库 IPC（单本「编辑信息」=书名+作者，多选「批量设置作者」，文件夹只能「重命名」），删除卷册进入库内 `trash/` 并可在回收站还原或清空。文件访问全部走 main 进程 + preload（`window.api.library/archive/convert/artifacts/deliver/webpush/upscale.*`），拖放路径通过 Electron `webUtils.getPathForFile(file)` 由 preload 暴露；数据层细节见 `docs/architecture.md`。`归档`（产物管理 + 投递）、`设备与邮箱`（SMTP 配置）、`转换设置`、`网页推送`（Send to Kindle 网页通道着陆/设置页，对应 `src/main/webpush.ts`）均已是真实 UI，挂在侧边栏「Kindle 推送」组（注意：侧边栏渲染用 `sidebarGroups` 而非 `primaryNav`）。`扩展功能`（extensions，icon Puzzle）已接入 waifu2x 引擎状态、阅读增强参数和缓存管理。`设计组件 / 基础规范` 是开发期工具页，**仅 dev 构建可见**（侧栏 `groupDevMode` 与视图都按 `import.meta.env.DEV` 门控），视图代码在 `src/renderer/src/dev/Showcase.tsx`（经 `React.lazy` 加载，生产包不打包，连同 recharts 等重依赖一并剔除）。已移除的导航占位：`导入收件箱`、`投递记录`、`添加资源库`；`转换队列` 功能已并入转换活动浮窗，不再有独立导航项。
...
**桌面化交互约定（这是应用不是网页）**：全局 `user-select: none`（输入类控件例外；个别需选中处用 Tailwind `select-text` 放开），网页式快捷键（刷新/缩放/打印）已在主进程 `before-input-event` 拦掉——不要重新放开或新增依赖浏览器默认行为的交互。`Cmd/Ctrl+R` 已复用为「编辑选中项」（主进程转发 `app:rename-selected` IPC，渲染层按单选项映射：书→编辑信息弹窗(书名+作者，散卷/夹内一致)、文件夹→重命名），`Delete/Backspace` 删除选中卷/部。空状态统一用可配置的 `PageEmpty`；「列一批东西 + 批量操作」的弹窗（导入卷册、库内回收站等）复用 `components/EntityListDialog.tsx`，不要再各写一套。左右布局组件必须防横向溢出：外层和文本区用 `min-w-0`，长标题/路径/说明用截断或折行，右侧按钮/菜单等交互入口保持 `shrink-0` 或独立操作区，不给操作区设置会裁剪按钮文案的固定窄宽/`max-width`；优先压缩信息文本而不是挤走或裁掉操作入口。应用各工作区（含阅读器）顶栏的纯图标工具统一使用 `Tooltip + Button variant="ghost" size="icon"`，Lucide 图标用 `text-muted-foreground` 与 `strokeWidth={1.75}`；有文字的操作使用 `size="sm"`。不要为单个工作区另设颜色或尺寸体系，警示/错误等语义状态除外。
...
已实现：Eagle 式 `.ctklib` 托管库包（`library.json` + `books/<bookId>/book.json` + `source.*`/`images/`）+ manifest 投影书架网格（文件夹/散卷、多级下钻、面包屑、排序、分组、文件夹重命名、每本书书名+作者编辑含批量设作者）+ 导入即复制成桶并可选删除源 + 导入目标选择（散卷/已有文件夹/新建文件夹）+ 外部拖放导入 + 库内 `trash/` 软删除/还原/清空 + 封面缩略图缓存 `userData/thumbs/`、卷册阅读器、库包路径持久化（`userData/settings.json`）、压缩包来源（`src/main/archive.ts`，7zip-bin 解 CBZ/ZIP/CBR/RAR/7z + 加密 zip + 多卷分卷到 `userData/extracted/`，safeStorage 加密的共享密码池，解压进度）、文档来源（`src/main/document.ts`，PDF.js + `@napi-rs/canvas` 渲染 PDF，图片型 EPUB 抽取页面到 `userData/documents/`）、阅读时 AI 放大（`src/main/upscale.ts` + waifu2x-ncnn-vulkan，图片目录/压缩包/图片型 EPUB，`userData/upscaled/` 缓存）、书转 Kindle 固定版式 EPUB（`src/main/convert.ts`，书名为单一字段、书自带书名+作者）、产物清单与归档（`src/main/artifacts.ts`，`userData/artifacts.json` + `userData/converted/`）、**转换队列持久化**（`src/main/queue.ts`，`userData/queue.json`；关窗即退出应用使「关窗==重启」成立；重启后未完成任务（queued/converting）标为 `'interrupted'`、不自动跑，由启动 toast +活动浮窗角标提示用户「继续/不继续」，继续=整卷从 0 重跑；孤儿 `tmp_*` 清扫；options 入队时冻结）、SMTP 投递（`src/main/deliver.ts`，凭据 safeStorage 加密）、Send to Kindle 网页推送（`src/main/webpush.ts`，内嵌网页 + CDP 自动填文件，≤200MB）、App 层转换队列调度（`useConvertActivity` hook）。
...
- PDF 阅读增强与转换时图像增强
- 转换后自动投递（已明确暂缓；SMTP 约 50MB 的实用体积目标会迫使漫画严重压缩，当前优先网页推送）

...
**桌面化交互约定（这是应用不是网页）**：全局 `user-select: none`（输入类控件例外；个别需选中处用 Tailwind `select-text` 放开），网页式快捷键（刷新/缩放/打印）已在主进程 `before-input-event` 拦掉——不要重新放开或新增依赖浏览器默认行为的交互。`Cmd/Ctrl+R` 已复用为「重命名选中项」（主进程转发 `app:rename-selected` IPC，渲染层按单选项映射：卷→重命名弹窗、部→编辑信息），`Delete/Backspace` 删除选中卷/部。空状态统一用可配置的 `PageEmpty`；「列一批东西 + 批量操作」的弹窗（导入卷册、库内回收站等）复用 `components/EntityListDialog.tsx`，不要再各写一套。
